...
describe('UserProfileRepository tests', () => {
    ...
    it('Find current user propagates errors', () => propagatesErrorTest(() => repository.findCurrentUserProfile()));
    ...
    it('Uploades image', () => {
        ...
        return repository.uploadUserProfileImage(image).then(r => {
            ...
            expect(requestsFake.capturedUrl).to.equal(endpoints.currentUserProfileImageUpload);
            expect(requestsFake.capturedFormData).to.deep.equal(expectedForm);
        });
    });

    it('Image upload propagates errors', () => {
        let image = new Blob(["Complex image"], { type: 'text/plain', name: 'image' });
        propagatesErrorTest(() => repository.uploadUserProfileImage(image));
    });
    ...
    it('Updates profile', () => {
        ...
        return repository.updateUserProfile(userProfileUpdate).then(r => {
            expect(r.success).to.equal(true);
            
            expect(requestsFake.capturedUrl).to.equal(endpoints.currentUserProfile);
            expect(requestsFake.capturedData).to.deep.equal(userProfileUpdate);
        });
    });
    ...
    it('Updates password', () => {
        ...
        return repository.updateUserPassword(passwordUpdate).then(r => {
            expect(r.success).to.equal(true);

            expect(requestsFake.capturedUrl).to.equal(endpoints.passwordUpdate);
            expect(requestsFake.capturedData).to.deep.equal(passwordUpdate);
        });
    });
});


function propagatesErrorTest(promiseSupplier) {
    let expectedException = "USER_DOES_NOT_EXIST";
    requestsFake.expectedResponse = Response.failure([expectedException]);

    return promiseSupplier().then(r => {
